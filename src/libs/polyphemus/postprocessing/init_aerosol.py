...
for i in range(config.Nbins):
    dry_section_diameter[i] = sqrt(dry_bound_diameter[i] * dry_bound_diameter[i + 1])
    dry_section_masses[i] = sqrt(dry_bound_masses[i] * dry_bound_masses[i + 1])


# PM limits.
Npm25 = argmin(abs(dry_bound_diameter - 2.5))
Npm10 = argmin(abs(dry_bound_diameter - 10.0))

# Options.
compute_pm10 = (
    compute_pm25
) = compute_tot_species = compute_bins = compute_granulo = False
...
if (
    compute_pm10
    or compute_pm25
    or compute_tot_species
    or compute_bins
    or compute_granulo
):
    for j in range(Nsim):
        if compute_pm10 or compute_pm25 or compute_bins or compute_granulo:
            sum_bin = zeros(
                [config.Nbins, config.Nt, config.Nz, config.Ny, config.Nx], dtype="d"
            )
        for ispecies in aerosol_species:
            ...
            for i in range(config.Nbins):
                ...
                sim = io.load_binary(
                    file_s, [config.Nt, config.Nz, config.Ny, config.Nx]
                )
                sum_species += sim
                if compute_pm10 or compute_pm25 or compute_bins or compute_granulo:
                    sum_bin[i] += sim
            ...
            io.save_binary(
                sum_species, os.path.join(directory_list[j], ispecies + ".bin")
            )
        ...
        if compute_pm10:
            ...
            io.save_binary(sum_pm10, os.path.join(directory_list[j], "PM10.bin"))

        if compute_pm25:
            sum_pm25 = sum(sum_bin[0:Npm25], 0)
            io.save_binary(sum_pm25, os.path.join(directory_list[j], "PM2.5.bin"))
        ...
        if compute_granulo:
            for i in range(config.Nbins):
                io.save_binary(
                    sum_bin[i] / dry_section_masses[i],
                    os.path.join(
                        directory_list[j], "number_" + str(i + bin_index_shift) + ".bin"
                    ),
                )
