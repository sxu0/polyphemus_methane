...
class ConfigPolair3D:
    ...
    def GetConfigVariable(self, model_index, ensemble_program):
        ...
        if param_dict["kz"] == "tm":
            kz_file = (
                function.create_output_dir(
                    "Kz_TM", ensemble_program.dependency["Kz_TM"], param_dict
                )
                + "/Kz_TM.bin"
            )
        ...
        vertical_distribution = (
            "vertical_distribution-" + ground_emission + "-" + Nz + ".dat"
        )
        if ground_emission == "ground":
            ...
            if Nz == "5":
                ...
                raise Exception, 'The level number: "' + Nz + '" is unknown.'
        else:
            raise Exception, 'The ground emission parameter: "' + ground_emission + '" is unknown.'
        Nz_in = str(int(Nz) + 1)
        # Photolysis
    ...
    def GetPerturbedFieldDict(self, model_index, ensemble_program):
        ...
        if (
            "bc_O3" not in param_dict.keys()
            and "bc_NOx" not in param_dict.keys()
            and "bc_VOC" not in param_dict.keys()
        ):
        ...
        if (
            "emission_NOx" not in param_dict.keys()
            and "emission_VOC" not in param_dict.keys()
            and "bio" in param_dict.keys()
        ) or param_dict["ground_emission"] == "ground":
            field_list.remove("VolumeEmission")
        if (
            "bio" not in param_dict.keys()
            and "emission_NOx" not in param_dict.keys()
            and "emission_VOC" not in param_dict.keys()
        ):
        ...
        temperature = function.get_perturbed_field(
            "temperature", param_dict, "Temperature"
        )
        wind_angle = function.get_perturbed_field("wind_angle", param_dict, "WindAngle")
        wind_velocity = function.get_perturbed_field(
            "wind_velocity", param_dict, "WindModule"
        )
        vertical_diff = function.get_perturbed_field(
            "vertical_diff", param_dict, "VerticalDiffusionCoefficient"
        )
        bc_O3 = function.get_perturbed_field("bc_O3", param_dict, "O3")
        bc_NOx = function.get_perturbed_field("bc_NOx", param_dict, NOx)
        bc_VOC = function.get_perturbed_field("bc_VOC", param_dict, VOC_species_bc)
        bio = function.get_perturbed_field("bio", param_dict, bio_species_emission)
        emission_NOx = function.get_perturbed_field("emission_NOx", param_dict, NOx)
        emission_VOC = function.get_perturbed_field(
            "emission_NOx", param_dict, VOC_species_emission
        )
        dep = function.get_perturbed_field("dep", param_dict, dep_species)
        photolysis = function.get_perturbed_field(
            "photolysis", param_dict, photolysis_species
        )
        emission = emission_NOx + emission_VOC

        result = {
            "%field_list%": string_field_list,
            "%temperature_perturbation%": temperature,
            "%wind_velocity_perturbation%": wind_velocity,
            "%wind_angle_perturbation%": wind_angle,
            "%vertical_diffusion_coefficient_perturbation%": vertical_diff,
