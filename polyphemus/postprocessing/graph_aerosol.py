...
additional_content = [
    ...
    ("[directory_list]", "", "directory_list", "StringSection"),
...
if config.graphs_at_station:
    station = io.load_station(
        config.station_file, config.station_file_type, config.station
    )
else:
...
# Chemical species.
if (
    "pie_chart" in config.graph_type
    or "bar_chart" in config.graph_type
    or "temporal" in config.graph_type
):
    PM_species = []
    for i in range(Nsim):
        try:
                ...
                if aerosol_species[ispecies] == "organics":
                    ...
                    for species in config.organics:
                        file_in = os.path.join(
                            config.directory_list[i], species + ".bin"
                        )
                        tmp = io.load_binary(
                            file_in, [config.Nt, config.Nz, config.Ny, config.Nx]
                        )[:, 0, :]
                        if config.graphs_at_station:
                            total += observation.get_simulated_at_station(
                                origin, delta, tmp, station
                            )

                        else:
                            total += tmp[
                                :, j_range[0] : j_range[1], i_range[0] : i_range[1]
                            ]
                    PM_species.append(total)
                else:
                    file_in = os.path.join(
                        config.directory_list[i], aerosol_species[ispecies] + ".bin"
                    )
                    tmp = io.load_binary(
                        file_in, [config.Nt, config.Nz, config.Ny, config.Nx]
                    )[:, 0, :]
                    if config.graphs_at_station:
                        PM_species.append(
                            observation.get_simulated_at_station(
                                origin, delta, tmp, station
                            )
                        )
                    else:
                        PM_species.append(
                            tmp[:, j_range[0] : j_range[1], i_range[0] : i_range[1]]
                        )
        except:
            raise Exception, "You have to launch init_aerosol.py with computation " + "of total species activated (option total_species)."
...
if "mass_distribution" in config.graph_type:
    ...
    try:
        ...
        raise Exception, "You have to launch init_aerosol.py with computation " + "of total bins activated (option total_bins)."
...
if "number_distribution" in config.graph_type:
    ...
    try:
        ...
        raise Exception, "You have to launch init_aerosol.py with computation " + "of granulometry activated (option granulometry)."

# Diameters.
if (
    "mass_distribution" in config.graph_type
    or "number_distribution" in config.graph_type
):
    ...
    for i in range(config.Nbins):
        dry_section_diameter.append(
            sqrt(dry_bound_diameter[i] * dry_bound_diameter[i + 1])
        )
...
if "pie_chart" in config.graph_type:
    for j in range(Nsim):
        ...
        pie(pm_composition, labels=species_names, colors=ccolors, autopct="%0.1f")
        title(legend_sim[j])
...
if "bar_chart" in config.graph_type:
    ...
    for j in range(Nsim):
        ...
        for i in range(Nspecies):
            sim_species = PM_species[j * Nspecies + i]
            pm_composition.append(sim_species.mean())
        pbar = bar(indbar + j * widthbar, pm_composition, widthbar, color=ccolors[j])
...
if "temporal" in config.graph_type:
    for j in range(Nsim):
        ...
        for i in range(Nspecies):
            sim_species = PM_species[j * Nspecies + i]
            if config.graphs_at_station:
                sim_to_plot.append(sim_to_plot[len(sim_to_plot) - 1] + sim_species)
            else:
                sim_to_plot.append(
                    sim_to_plot[len(sim_to_plot) - 1] + mean(mean(sim_species, 2), 1)
                )

            sim_lines.append(
                display.segplot_date(
                    date2num(sim_dates), sim_to_plot[len(sim_to_plot) - 1], "-", 0.5
                )
            )
        ...
        if Ndays < 62:
            locator = DayLocator(interval=ceil(float(Ndays) / 7.0))
...
if "mass_distribution" in config.graph_type:
    ...
    for j in range(Nsim):
        ...
        if config.log_plot:
            mass_distr_line = display.segplot_logx(
                dry_section_diameter, mass_distribution, sim_fmt[j], 10
            )
            mass_distr_lines.append(mass_distr_line)
        else:
            mass_distr_line = display.segplot(
                dry_section_diameter, mass_distribution, sim_fmt[j], 10
            )
            mass_distr_lines.append(mass_distr_line)
...
if "number_distribution" in config.graph_type:
    ...
    for j in range(Nsim):
        ...
        if config.log_plot:
            num_distr_line = display.segplot_logx(
                dry_section_diameter, num_distribution, sim_fmt[j], 10
            )
            num_distr_lines.append(num_distr_line)
        else:
            num_distr_line = display.segplot(
                dry_section_diameter, num_distribution, sim_fmt[j], 10
            )
            num_distr_lines.append(num_distr_line)
