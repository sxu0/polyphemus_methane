...
if len(sys.argv) != 2:
    print "Please provide one configuration file to " + sys.argv[0] + "."
    sys.exit(1)

configuration_file = os.path.abspath(sys.argv[1])
if not os.path.isfile(configuration_file):
    print 'ERROR: unable to open the configuration file "%s".' % configuration_file
    sys.exit(1)
...
os.environ["compilation_option"] = configuration.GetString("compilation_option")
...
if main_configuration_file[-3:] == "cfg":
    ...
    date_min = get_date_min(main_configuration.GetString("model_configuration_file"))
else:
...
compile_run_command = os.path.join(
    os.path.join(os.environ["polyphemus_path"], "include/models/chimere/compile_run.py")
)
