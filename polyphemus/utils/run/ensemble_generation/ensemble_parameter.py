...
class EnsembleParameter:
    ...
    def LoadConfiguration(self, configuration_file):
        ...
        if not path.isdir(self.config.data_dir):
            raise Exception, 'Unable to find the directory "' + self.config.data_dir + '".'
        if not path.isdir(self.config.model_dir):
            raise Exception, 'Unable to find the directory "' + self.config.model_dir + '".'
    ...
    def GetDict(self, model_index=None):
        ...
        if len(self.id_ensemble) != self.Nmodel:
            raise Exception, "The length of your models identity list (" + str(
                len(self.id_ensemble)
            ) + ") must be equal to " + str(
                self.Nmodel
            ) + ". Please load or generate an ensemble."
        if model_index != None:
            if model_index >= self.Nmodel:
                raise Exception, 'The model index "' + str(
                    model_index
                ) + '" is too high. The model number must be less than ' + str(
                    self.Nmodel
                ) + "."
    ...
    def GetIdProgram(self, id_model, name):
        ...
        for n in name:
            if n not in self.name:
                raise Exception, 'The parameter name "' + n + '" is not' + ' in the parameters list of the file "' + self.config_filename + '".'
    ...
    def GetNormalUncertainty(self, value):
        ...
        return [0.0, -value / 2.0, +value / 2.0]
    ...
    def GetLogNormalUncertainty(self, value):
        ...
        if value <= 0:
            raise Exception, 'The value: "' + str(
                value
            ) + '" must be' + " higher than 0 for a log-normal law."
        beta = exp(0.5 * (0.5 * log(value)) ** 2)
        delta = 4.0 * beta ** 4 - 7.0 * beta ** 2 + 6.0 * beta - 3.0
        if 3.0 * beta - 1.0 - sqrt(delta) <= 0:
            raise Exception, 'The value :"' + str(value) + '" is not permitted.'
        a = 2.0 * log((3.0 * beta - 1 - sqrt(delta)) / 2.0) / log(value)
        b = 2.0 * log((3.0 * beta - 1 + sqrt(delta)) / 2.0) / log(value)
        ...
        return [1.0, value ** (a / 2.0), value ** (b / 2.0)]
    ...
    def ExtractSection(self, section):
        ...
        if hasattr(self.config, section):
            ...
            raise Exception, 'Unable to find the section "' + section + '" in the configuration file "' + self.config_filename + '".'
        N = len(self.name)
        if len(parameter_list[0]) != 0:
            ...
            for i in range(len(parameter_list)):
                ...
                if section in ["physics", "numerics"]:
                    ...
                    for s in tmp[1:]:
                        if s == "preprocessing":
                            self.preprocessing.append(self.name[i + N])
                            continue
                        # If there is an occurrence frequency.
                        if "(" in s:
                            number = talos.to_num(s.strip(",()-;"))
                            self.occurrence_frequency[i + N].append(number)
                    ...
                    if len(self.occurrence_frequency[i + N]) == 0:
                        for _ in range(self.parameter_size[i + N]):
                            self.occurrence_frequency[i + N].append(
                                100.0 / float(self.parameter_size[i + N])
                            )
                    # If the number of occurrence frequencies is wrong.
                    elif (
                        len(self.occurrence_frequency[i + N])
                        != self.parameter_size[i + N]
                    ):
                        raise Exception, "The number of occurrence " + "frequencies (" + str(
                            len(self.occurrence_frequency[i + N])
                        ) + ') for the parameter "' + self.name[
                            i + N
                        ] + '" must be equal to ' + str(
                            self.parameter_size[i + N]
                        ) + "."
                # If the section is 'input_data'.
                    ...
                    if type_perturbation == "normal":
                        ...
                        raise Exception, 'The type: "' + type_perturbation + '" in "' + self.config_filename + '"' + " is not supported."
    ...
    def ParameterChoice(self, index_parameter):
        ...
        if index_parameter >= self.Nparameter - input_data_length:
            raise Exception, "The index of the parameter must " + "equal or less than " + str(
                self.Nparameter - input_data_length
            ) + "."
        # The sum of occurrence frequencies must be equal to 100.
        if sum(self.occurrence_frequency[index_parameter]) != 100:
            raise Exception, 'The sum of the occurrence frequencies "' + str(
                self.occurrence_frequency[index_parameter]
            ) + '" must be equal to 100 for the parameter "' + self.name[
                index_parameter
            ] + '".'
    ...
    def GenerateIdModel(self):
        ...
        if "chemistry" in self.name:
            ...
            if self.value[index_chemistry][id_model[index_chemistry]] == "radm":
    ...
    def GenerateIdEnsemble(self):
        ...
        if self.Nmodel > Nmodel_possible:
            raise Exception, "Unable to generate an ensemble of " + str(
                self.Nmodel
            ) + " members. There are only " + str(Nmodel_possible) + " possibilities."
    ...
    def CheckParameterFrequency(self):
        ...
        for i in range(self.Nparameter):
            ...
            if count != self.parameter_size[i]:
                print (
                    'Checks parameter frequency : the index values "'
                    + str(index)
                    + '" is not present in the id_ensemble'
                    + ' for the parameter "'
                    + self.name[i]
                    + '".'
                )
    ...
    def AddModel(self, id_model):
        ...
        if (len(self.id_ensemble)) >= self.Nmodel:
            raise Exception, "Unable to add a model, the Nmodel is " + "already equal to " + str(
                self.Nmodel
    ...
    def IdModelToString(self, id_model):
        ...
        if len(id_model) != self.Nparameter:
            raise Exception, "The number of parameters is wrong (" + str(
                len(id_model)
            ) + "). It must be equal to: " + str(self.Nparameter) + "."
        result = ""
    ...
    def IdModelToList(self, id_model):
        ...
        for i in range(self.Nparameter):
            if result[i] >= self.parameter_size[i]:
                raise Exception, 'The number of values in the parameter "' + self.name[
                    i
                ] + '" must be less than ' + str(
                    self.parameter_size[i]
                ) + " (instead of " + str(
                    result[i]
                ) + " )."
        return result
    ...
    def ReadIdEnsembleFile(self, filename, copy=False):
        ...
        if not path.isfile(filename):
            raise Exception, 'Unable to open file "' + filename + '" and read id_ensemble.'
    ...
    def ShowModelParameter(self, id_model):
        ...
        for i in range(self.Nparameter):
            if isinstance(self.value[i], tuple):
                print (
                    self.name[i] + " : " + str(self.value[i][0][model[i]])
                ) + " - " + self.value[i][-1]
            else:
